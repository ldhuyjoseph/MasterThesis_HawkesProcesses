...
\usepackage{hyperref}
% ------------------------------------------------------------
% Reproducibility links (GitHub)
% ------------------------------------------------------------
\newcommand{\githubrepo}{https://github.com/ldhuyjoseph/MasterThesis_paper-code}

\newcommand{\figlink}[1]{\href{\githubrepo/numerical_experiments/outputs/figures/#1}{[GitHub]}}
\newcommand{\tablelink}[1]{\href{\githubrepo/numerical_experiments/outputs/tables/#1}{[GitHub]}}
...
Within Figure~\ref{fig:onepath}, the top panel presents a histogram of event arrivals across time bins. Arrivals cluster in visible bursts — a signature of the self-exciting mechanism. The middle panel plots the counting process $N$ against $\mathbb{E}[N(\cdot) \mid \lambda_0]$; the simulated path fluctuates around the mean but shows periods of accelerated growth corresponding to the clusters above. The bottom panel shows the intensity process $\lambda$ and $\mathbb{E}[\lambda(\cdot) \mid \lambda_0]$, where the most pronounced spikes in intensity correspond exactly to the densest clusters of arrivals visible in the top panel. \begin{figure}[H]
    ...
    \caption{
One simulated sample path of the Hawkes process $(N,\lambda)$.
Reproducible output available at 
\figlink{Fig7_1_dz13_one_path.png}.
}
    \label{fig:onepath}
\end{figure}
\noindent
...
\begin{figure}
    ...
    \caption{
Comparison between theoretical and simulated moments of $N_t$ and $\lambda_t$.
Reproducible output available at 
\figlink{Fig7_2_dz13_monte_carlo_comparison.png}.
}
...
\centering
\caption{
Comparison between theoretical formulas and Monte Carlo simulation results
($T=1,\dots,20$, $M=100{,}000$).
Full reproducible table available at 
\tablelink{Table7_1_dz13_monte_carlo.csv}.
}
\label{tab:moment}
